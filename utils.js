...
// this is used to determine if the bars should have number labels or not
function isMobile() {
  var check = false;
  (function (a) {
    if (
...
let permanentHighlight = [null, null];

function addPermanentHighlight(idx, color) {
  permanentHighlight = [idx, color];
}

function clearPermanentHighlight() {
  permanentHighlight = [null, null];
}
...
function renderBars(arr) {
  ...
  arr.forEach((num, idx) => {
    ...
    if (idx === permanentHighlight[0]) {
      bar.style.backgroundColor = permanentHighlight[1];
    }
